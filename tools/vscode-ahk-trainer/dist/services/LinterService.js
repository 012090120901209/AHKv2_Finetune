...
class LinterService {
    ...
    toVSCodeDiagnostics(diagnostics, uri) {
        return diagnostics.map(d => {
            const range = new vscode.Range(new vscode.Position(Math.max(0, d.line - 1), Math.max(0, d.column - 1)), new vscode.Position(Math.max(0, (d.endLine || d.line) - 1), Math.max(0, (d.endColumn || d.column) - 1)));
            const vscodeDiagnostic = new vscode.Diagnostic(range, d.message, this.toVSCodeSeverity(d.severity));
...
//# sourceMappingURL=LinterService.js.map