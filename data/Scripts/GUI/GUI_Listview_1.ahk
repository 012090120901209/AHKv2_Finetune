...
GuiClose: ; Indicate that the script should exit automatically when the window is closed.
    GuiClose()
    ExitApp()
    MyListView(A_GuiEvent := "", A_GuiControl := "", Info := "", *) {
        if (A_GuiEvent = "DoubleClick") {
            RowText := ogcListViewNameSizeKB.GetText(Info) ; Get the text from the row's first field. ToolTip("You double-clicked row number " Info ". Text: `"" RowText "`"") }
        }
        GuiClose(*) {
            ExitApp()
        }
