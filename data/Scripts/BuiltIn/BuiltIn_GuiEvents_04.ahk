#Requires AutoHotkey v2.0
#SingleInstance Force

/**
 * BuiltIn_GuiEvents_04.ahk - Input Validation Events
 * 
 * This file demonstrates input validation using events in AutoHotkey v2.
 * Topics covered:
 * - Real-time validation
 * - Form validation
 * - Custom validators
 * - Error display
 * - Validation messages
 * - Multi-field validation
 * - Validation rules
 * 
 * @author AutoHotkey Community
 * @version 2.0
 * @date 2024
 */


; =============================================================================
; Example 1: Real-time validation
; =============================================================================

/**
 * Demonstrates real-time validation with practical examples
 * Shows event handling patterns and best practices
 */
Example1_Realtimevalidation() {
    myGui := Gui(, "Real-time validation Demo")
    myGui.BackColor := "White"

    myGui.SetFont("s11 Bold")
    myGui.Add("Text", "x20 y20 w560", "Real-time validation Event Handling")
    myGui.SetFont("s9 Norm")

    ; Event log
    myGui.Add("Text", "x20 y55", "Event Log:")
    eventLog := myGui.Add("Edit", "x20 y75 w560 h200 ReadOnly Multi")
    eventLog.Value := "Event log initialized...\n"

    eventCount := 0

    LogEvent(msg) {
        timestamp := FormatTime(A_Now, "HH:mm:ss")
        eventCount++
        eventLog.Value .= Format("[{1}] Event #{2}: {3}\n", timestamp, eventCount, msg)
        ControlSend("{End}", eventLog)
    }

    ; Interactive elements
    myGui.Add("Text", "x20 y285", "Interact with these controls to trigger events:")

    ; Buttons
    btn1 := myGui.Add("Button", "x20 y310 w130", "Button 1")
    btn2 := myGui.Add("Button", "x160 y310 w130", "Button 2")
    btn3 := myGui.Add("Button", "x300 y310 w130", "Button 3")

    btn1.OnEvent("Click", (*) => LogEvent("Button 1 clicked"))
    btn2.OnEvent("Click", (*) => LogEvent("Button 2 clicked"))
    btn3.OnEvent("Click", (*) => LogEvent("Button 3 clicked"))

    ; Edit box with change detection
    myGui.Add("Text", "x20 y350", "Type to trigger change events:")
    editBox := myGui.Add("Edit", "x20 y370 w280")
    editBox.OnEvent("Change", (*) => LogEvent("Edit box changed: " editBox.Value))

    ; Checkbox
    chk := myGui.Add("Checkbox", "x320 y370", "Enable feature")
    chk.OnEvent("Click", (*) => LogEvent("Checkbox " (chk.Value ? "checked" : "unchecked")))

    ; Clear log button
    myGui.Add("Button", "x440 y310 w140", "Clear Log").OnEvent("Click", ClearLog)

    ClearLog(*) {
        eventLog.Value := "Event log cleared...\n"
        eventCount := 0
        LogEvent("Log cleared")
    }

    myGui.Add("Button", "x20 y410 w560", "Close").OnEvent("Click", (*) => myGui.Destroy())

    LogEvent("Window created and shown")
    myGui.Show("w600 h460")
}


; =============================================================================
; Example 2: Form validation
; =============================================================================

/**
 * Demonstrates form validation with practical examples
 * Shows event handling patterns and best practices
 */
Example2_Formvalidation() {
    myGui := Gui(, "Form validation Demo")
    myGui.BackColor := "White"

    myGui.SetFont("s11 Bold")
    myGui.Add("Text", "x20 y20 w560", "Form validation Event Handling")
    myGui.SetFont("s9 Norm")

    ; Event log
    myGui.Add("Text", "x20 y55", "Event Log:")
    eventLog := myGui.Add("Edit", "x20 y75 w560 h200 ReadOnly Multi")
    eventLog.Value := "Event log initialized...\n"

    eventCount := 0

    LogEvent(msg) {
        timestamp := FormatTime(A_Now, "HH:mm:ss")
        eventCount++
        eventLog.Value .= Format("[{1}] Event #{2}: {3}\n", timestamp, eventCount, msg)
        ControlSend("{End}", eventLog)
    }

    ; Interactive elements
    myGui.Add("Text", "x20 y285", "Interact with these controls to trigger events:")

    ; Buttons
    btn1 := myGui.Add("Button", "x20 y310 w130", "Button 1")
    btn2 := myGui.Add("Button", "x160 y310 w130", "Button 2")
    btn3 := myGui.Add("Button", "x300 y310 w130", "Button 3")

    btn1.OnEvent("Click", (*) => LogEvent("Button 1 clicked"))
    btn2.OnEvent("Click", (*) => LogEvent("Button 2 clicked"))
    btn3.OnEvent("Click", (*) => LogEvent("Button 3 clicked"))

    ; Edit box with change detection
    myGui.Add("Text", "x20 y350", "Type to trigger change events:")
    editBox := myGui.Add("Edit", "x20 y370 w280")
    editBox.OnEvent("Change", (*) => LogEvent("Edit box changed: " editBox.Value))

    ; Checkbox
    chk := myGui.Add("Checkbox", "x320 y370", "Enable feature")
    chk.OnEvent("Click", (*) => LogEvent("Checkbox " (chk.Value ? "checked" : "unchecked")))

    ; Clear log button
    myGui.Add("Button", "x440 y310 w140", "Clear Log").OnEvent("Click", ClearLog)

    ClearLog(*) {
        eventLog.Value := "Event log cleared...\n"
        eventCount := 0
        LogEvent("Log cleared")
    }

    myGui.Add("Button", "x20 y410 w560", "Close").OnEvent("Click", (*) => myGui.Destroy())

    LogEvent("Window created and shown")
    myGui.Show("w600 h460")
}


; =============================================================================
; Example 3: Custom validators
; =============================================================================

/**
 * Demonstrates custom validators with practical examples
 * Shows event handling patterns and best practices
 */
Example3_Customvalidators() {
    myGui := Gui(, "Custom validators Demo")
    myGui.BackColor := "White"

    myGui.SetFont("s11 Bold")
    myGui.Add("Text", "x20 y20 w560", "Custom validators Event Handling")
    myGui.SetFont("s9 Norm")

    ; Event log
    myGui.Add("Text", "x20 y55", "Event Log:")
    eventLog := myGui.Add("Edit", "x20 y75 w560 h200 ReadOnly Multi")
    eventLog.Value := "Event log initialized...\n"

    eventCount := 0

    LogEvent(msg) {
        timestamp := FormatTime(A_Now, "HH:mm:ss")
        eventCount++
        eventLog.Value .= Format("[{1}] Event #{2}: {3}\n", timestamp, eventCount, msg)
        ControlSend("{End}", eventLog)
    }

    ; Interactive elements
    myGui.Add("Text", "x20 y285", "Interact with these controls to trigger events:")

    ; Buttons
    btn1 := myGui.Add("Button", "x20 y310 w130", "Button 1")
    btn2 := myGui.Add("Button", "x160 y310 w130", "Button 2")
    btn3 := myGui.Add("Button", "x300 y310 w130", "Button 3")

    btn1.OnEvent("Click", (*) => LogEvent("Button 1 clicked"))
    btn2.OnEvent("Click", (*) => LogEvent("Button 2 clicked"))
    btn3.OnEvent("Click", (*) => LogEvent("Button 3 clicked"))

    ; Edit box with change detection
    myGui.Add("Text", "x20 y350", "Type to trigger change events:")
    editBox := myGui.Add("Edit", "x20 y370 w280")
    editBox.OnEvent("Change", (*) => LogEvent("Edit box changed: " editBox.Value))

    ; Checkbox
    chk := myGui.Add("Checkbox", "x320 y370", "Enable feature")
    chk.OnEvent("Click", (*) => LogEvent("Checkbox " (chk.Value ? "checked" : "unchecked")))

    ; Clear log button
    myGui.Add("Button", "x440 y310 w140", "Clear Log").OnEvent("Click", ClearLog)

    ClearLog(*) {
        eventLog.Value := "Event log cleared...\n"
        eventCount := 0
        LogEvent("Log cleared")
    }

    myGui.Add("Button", "x20 y410 w560", "Close").OnEvent("Click", (*) => myGui.Destroy())

    LogEvent("Window created and shown")
    myGui.Show("w600 h460")
}


; =============================================================================
; Example 4: Error display
; =============================================================================

/**
 * Demonstrates error display with practical examples
 * Shows event handling patterns and best practices
 */
Example4_Errordisplay() {
    myGui := Gui(, "Error display Demo")
    myGui.BackColor := "White"

    myGui.SetFont("s11 Bold")
    myGui.Add("Text", "x20 y20 w560", "Error display Event Handling")
    myGui.SetFont("s9 Norm")

    ; Event log
    myGui.Add("Text", "x20 y55", "Event Log:")
    eventLog := myGui.Add("Edit", "x20 y75 w560 h200 ReadOnly Multi")
    eventLog.Value := "Event log initialized...\n"

    eventCount := 0

    LogEvent(msg) {
        timestamp := FormatTime(A_Now, "HH:mm:ss")
        eventCount++
        eventLog.Value .= Format("[{1}] Event #{2}: {3}\n", timestamp, eventCount, msg)
        ControlSend("{End}", eventLog)
    }

    ; Interactive elements
    myGui.Add("Text", "x20 y285", "Interact with these controls to trigger events:")

    ; Buttons
    btn1 := myGui.Add("Button", "x20 y310 w130", "Button 1")
    btn2 := myGui.Add("Button", "x160 y310 w130", "Button 2")
    btn3 := myGui.Add("Button", "x300 y310 w130", "Button 3")

    btn1.OnEvent("Click", (*) => LogEvent("Button 1 clicked"))
    btn2.OnEvent("Click", (*) => LogEvent("Button 2 clicked"))
    btn3.OnEvent("Click", (*) => LogEvent("Button 3 clicked"))

    ; Edit box with change detection
    myGui.Add("Text", "x20 y350", "Type to trigger change events:")
    editBox := myGui.Add("Edit", "x20 y370 w280")
    editBox.OnEvent("Change", (*) => LogEvent("Edit box changed: " editBox.Value))

    ; Checkbox
    chk := myGui.Add("Checkbox", "x320 y370", "Enable feature")
    chk.OnEvent("Click", (*) => LogEvent("Checkbox " (chk.Value ? "checked" : "unchecked")))

    ; Clear log button
    myGui.Add("Button", "x440 y310 w140", "Clear Log").OnEvent("Click", ClearLog)

    ClearLog(*) {
        eventLog.Value := "Event log cleared...\n"
        eventCount := 0
        LogEvent("Log cleared")
    }

    myGui.Add("Button", "x20 y410 w560", "Close").OnEvent("Click", (*) => myGui.Destroy())

    LogEvent("Window created and shown")
    myGui.Show("w600 h460")
}


; =============================================================================
; Example 5: Validation messages
; =============================================================================

/**
 * Demonstrates validation messages with practical examples
 * Shows event handling patterns and best practices
 */
Example5_Validationmessages() {
    myGui := Gui(, "Validation messages Demo")
    myGui.BackColor := "White"

    myGui.SetFont("s11 Bold")
    myGui.Add("Text", "x20 y20 w560", "Validation messages Event Handling")
    myGui.SetFont("s9 Norm")

    ; Event log
    myGui.Add("Text", "x20 y55", "Event Log:")
    eventLog := myGui.Add("Edit", "x20 y75 w560 h200 ReadOnly Multi")
    eventLog.Value := "Event log initialized...\n"

    eventCount := 0

    LogEvent(msg) {
        timestamp := FormatTime(A_Now, "HH:mm:ss")
        eventCount++
        eventLog.Value .= Format("[{1}] Event #{2}: {3}\n", timestamp, eventCount, msg)
        ControlSend("{End}", eventLog)
    }

    ; Interactive elements
    myGui.Add("Text", "x20 y285", "Interact with these controls to trigger events:")

    ; Buttons
    btn1 := myGui.Add("Button", "x20 y310 w130", "Button 1")
    btn2 := myGui.Add("Button", "x160 y310 w130", "Button 2")
    btn3 := myGui.Add("Button", "x300 y310 w130", "Button 3")

    btn1.OnEvent("Click", (*) => LogEvent("Button 1 clicked"))
    btn2.OnEvent("Click", (*) => LogEvent("Button 2 clicked"))
    btn3.OnEvent("Click", (*) => LogEvent("Button 3 clicked"))

    ; Edit box with change detection
    myGui.Add("Text", "x20 y350", "Type to trigger change events:")
    editBox := myGui.Add("Edit", "x20 y370 w280")
    editBox.OnEvent("Change", (*) => LogEvent("Edit box changed: " editBox.Value))

    ; Checkbox
    chk := myGui.Add("Checkbox", "x320 y370", "Enable feature")
    chk.OnEvent("Click", (*) => LogEvent("Checkbox " (chk.Value ? "checked" : "unchecked")))

    ; Clear log button
    myGui.Add("Button", "x440 y310 w140", "Clear Log").OnEvent("Click", ClearLog)

    ClearLog(*) {
        eventLog.Value := "Event log cleared...\n"
        eventCount := 0
        LogEvent("Log cleared")
    }

    myGui.Add("Button", "x20 y410 w560", "Close").OnEvent("Click", (*) => myGui.Destroy())

    LogEvent("Window created and shown")
    myGui.Show("w600 h460")
}


; =============================================================================
; Example 6: Multi-field validation
; =============================================================================

/**
 * Demonstrates multi-field validation with practical examples
 * Shows event handling patterns and best practices
 */
Example6_Multifieldvalidation() {
    myGui := Gui(, "Multi-field validation Demo")
    myGui.BackColor := "White"

    myGui.SetFont("s11 Bold")
    myGui.Add("Text", "x20 y20 w560", "Multi-field validation Event Handling")
    myGui.SetFont("s9 Norm")

    ; Event log
    myGui.Add("Text", "x20 y55", "Event Log:")
    eventLog := myGui.Add("Edit", "x20 y75 w560 h200 ReadOnly Multi")
    eventLog.Value := "Event log initialized...\n"

    eventCount := 0

    LogEvent(msg) {
        timestamp := FormatTime(A_Now, "HH:mm:ss")
        eventCount++
        eventLog.Value .= Format("[{1}] Event #{2}: {3}\n", timestamp, eventCount, msg)
        ControlSend("{End}", eventLog)
    }

    ; Interactive elements
    myGui.Add("Text", "x20 y285", "Interact with these controls to trigger events:")

    ; Buttons
    btn1 := myGui.Add("Button", "x20 y310 w130", "Button 1")
    btn2 := myGui.Add("Button", "x160 y310 w130", "Button 2")
    btn3 := myGui.Add("Button", "x300 y310 w130", "Button 3")

    btn1.OnEvent("Click", (*) => LogEvent("Button 1 clicked"))
    btn2.OnEvent("Click", (*) => LogEvent("Button 2 clicked"))
    btn3.OnEvent("Click", (*) => LogEvent("Button 3 clicked"))

    ; Edit box with change detection
    myGui.Add("Text", "x20 y350", "Type to trigger change events:")
    editBox := myGui.Add("Edit", "x20 y370 w280")
    editBox.OnEvent("Change", (*) => LogEvent("Edit box changed: " editBox.Value))

    ; Checkbox
    chk := myGui.Add("Checkbox", "x320 y370", "Enable feature")
    chk.OnEvent("Click", (*) => LogEvent("Checkbox " (chk.Value ? "checked" : "unchecked")))

    ; Clear log button
    myGui.Add("Button", "x440 y310 w140", "Clear Log").OnEvent("Click", ClearLog)

    ClearLog(*) {
        eventLog.Value := "Event log cleared...\n"
        eventCount := 0
        LogEvent("Log cleared")
    }

    myGui.Add("Button", "x20 y410 w560", "Close").OnEvent("Click", (*) => myGui.Destroy())

    LogEvent("Window created and shown")
    myGui.Show("w600 h460")
}


; =============================================================================
; Example 7: Validation rules
; =============================================================================

/**
 * Demonstrates validation rules with practical examples
 * Shows event handling patterns and best practices
 */
Example7_Validationrules() {
    myGui := Gui(, "Validation rules Demo")
    myGui.BackColor := "White"

    myGui.SetFont("s11 Bold")
    myGui.Add("Text", "x20 y20 w560", "Validation rules Event Handling")
    myGui.SetFont("s9 Norm")

    ; Event log
    myGui.Add("Text", "x20 y55", "Event Log:")
    eventLog := myGui.Add("Edit", "x20 y75 w560 h200 ReadOnly Multi")
    eventLog.Value := "Event log initialized...\n"

    eventCount := 0

    LogEvent(msg) {
        timestamp := FormatTime(A_Now, "HH:mm:ss")
        eventCount++
        eventLog.Value .= Format("[{1}] Event #{2}: {3}\n", timestamp, eventCount, msg)
        ControlSend("{End}", eventLog)
    }

    ; Interactive elements
    myGui.Add("Text", "x20 y285", "Interact with these controls to trigger events:")

    ; Buttons
    btn1 := myGui.Add("Button", "x20 y310 w130", "Button 1")
    btn2 := myGui.Add("Button", "x160 y310 w130", "Button 2")
    btn3 := myGui.Add("Button", "x300 y310 w130", "Button 3")

    btn1.OnEvent("Click", (*) => LogEvent("Button 1 clicked"))
    btn2.OnEvent("Click", (*) => LogEvent("Button 2 clicked"))
    btn3.OnEvent("Click", (*) => LogEvent("Button 3 clicked"))

    ; Edit box with change detection
    myGui.Add("Text", "x20 y350", "Type to trigger change events:")
    editBox := myGui.Add("Edit", "x20 y370 w280")
    editBox.OnEvent("Change", (*) => LogEvent("Edit box changed: " editBox.Value))

    ; Checkbox
    chk := myGui.Add("Checkbox", "x320 y370", "Enable feature")
    chk.OnEvent("Click", (*) => LogEvent("Checkbox " (chk.Value ? "checked" : "unchecked")))

    ; Clear log button
    myGui.Add("Button", "x440 y310 w140", "Clear Log").OnEvent("Click", ClearLog)

    ClearLog(*) {
        eventLog.Value := "Event log cleared...\n"
        eventCount := 0
        LogEvent("Log cleared")
    }

    myGui.Add("Button", "x20 y410 w560", "Close").OnEvent("Click", (*) => myGui.Destroy())

    LogEvent("Window created and shown")
    myGui.Show("w600 h460")
}


; =============================================================================
; Main Menu - Example Launcher
; =============================================================================

/**
 * Creates a main menu to launch all examples
 */
ShowMainMenu() {
    menuGui := Gui(, "BuiltIn_GuiEvents_04.ahk - Examples")
    menuGui.BackColor := "White"

    menuGui.SetFont("s10 Bold")
    menuGui.Add("Text", "x20 y20 w360", "Input Validation Events")
    menuGui.SetFont("s9 Norm")

    menuGui.Add("Text", "x20 y50 w360", "Select an example to run:")

    menuGui.Add("Button", "x20 y80 w360", "Example 1: Real-time validation").OnEvent("Click", (*) => Example1_Realtimevalidation())
    menuGui.Add("Button", "x20 y110 w360", "Example 2: Form validation").OnEvent("Click", (*) => Example2_Formvalidation())
    menuGui.Add("Button", "x20 y140 w360", "Example 3: Custom validators").OnEvent("Click", (*) => Example3_Customvalidators())
    menuGui.Add("Button", "x20 y170 w360", "Example 4: Error display").OnEvent("Click", (*) => Example4_Errordisplay())
    menuGui.Add("Button", "x20 y200 w360", "Example 5: Validation messages").OnEvent("Click", (*) => Example5_Validationmessages())
    menuGui.Add("Button", "x20 y230 w360", "Example 6: Multi-field validation").OnEvent("Click", (*) => Example6_Multifieldvalidation())
    menuGui.Add("Button", "x20 y260 w360", "Example 7: Validation rules").OnEvent("Click", (*) => Example7_Validationrules())

    menuGui.Add("Button", "x20 y300 w360", "Exit").OnEvent("Click", (*) => ExitApp())

    menuGui.Show("w400 h350")
}

; Show the main menu
ShowMainMenu()
