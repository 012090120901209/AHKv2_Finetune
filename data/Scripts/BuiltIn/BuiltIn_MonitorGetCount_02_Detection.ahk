...
Example4_HistoryTracker() {
    ...
    ExportHistory(*) {
        ...
        exportText := "Monitor Count Change History`n"
        exportText .= "Generated: " FormatTime(, "yyyy-MM-dd HH:mm:ss") "`n`n"

        for entry in history {
            exportText .= "#" entry.Num " - "
            exportText .= FormatTime(entry.Timestamp, "yyyy-MM-dd HH:mm:ss") " - "
            exportText .= "Count: " entry.Count " - "
            exportText .= "Duration: " entry.Duration " - "
            exportText .= entry.ChangeType "`n"
        }
...
Example6_AlertSystem() {
    ...
    StopMonitoring(*) {
        ...
        isMonitoring := false
        LogMessage("Monitoring stopped after " numChecks " checks")
